...
@section('content')
    ...
    <div class="flex justify-between items-center">
        <div class="border-s-4 border-[#395670] text-3xl font-bold text-[#395670] px-3">Profile Pengguna</div>
    ...
    <?php
        $data = auth()->user();
    ?>
    <form action="{{ route('pengguna.user.update') }}" method="POST" class="flex justify-center items-center md:h-[70vh] text-[#395670] font-bold mt-7 md:mt-14 pb-14">
        @csrf
        @method('PUT')
        <div class="w-[93vw] md:w-[60vw] shadow-lg shadow-gray-400 p-9 rounded-lg">
            <div class="md:flex items-center gap-20">
                <div class="text-center flex flex-col items-center mb-10 md:mb-0 shadow md:shadow-none shadow-gray-400 py-3 md:py-0 rounded-lg md:rounded-none">
                ...
                <div class="">
                    <div class="flex flex-col">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $data->name) }}" class="rounded border border-[#395670]">
                    </div>
                    <div class="flex flex-col mt-3">
                        <label for="email">Pesan Masukan</label>
                        <input type="text" name="email" id="email" value="{{ old('email', $data->email) }}" class="rounded border border-[#395670]">
                    </div>
                    <div class="flex flex-col mt-3">
                        <label for="email">Password Baru</label>
                        <input type="text" name="password" id="password" value="{{ old('password') }}" class="rounded border border-[#395670]">
                    </div>
                    <div class="flex flex-col mt-3">
                        <label for="email">Konfirmasi Password Baru</label>
                        <input type="text" name="password_confirmation" id="password_confirmation" value="{{ old('password_confirmation') }}" class="rounded border border-[#395670]">
                    </div>
                </div>
            </div>
            <div class="flex justify-center items-center mt-7">
                <button type="submit" class="bg-[#247BA0] text-white text-sm rounded font-semibold py-2 px-20">UBAH</button>
            </div>
        </div>
    </div>
