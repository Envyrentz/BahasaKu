...
@section('content')
    <div class="border-s-4 border-[#395670] text-3xl font-bold text-[#395670] px-3">Ubah Data Pengguna</div>
    ...
    <form action="{{ route('pengguna.update', $data->id) }}" method="POST" enctype="multipart/form-data" class="w-full pb-7">
        @csrf
        @method('PUT')
        <div class="flex">
            <div class="flex flex-col md:gap-5 md:pe-5 w-full">
                ...
                <div class="md:flex md:gap-10">
                    <div class="basis-1/2 flex flex-col">
                        ...
                        <input type="text" name="name" id="name" class="rounded" value="{{ old('name', $data->name) }}" required>
                    </div>
                    <div class="basis-1/2 flex flex-col mt-2 md:mt-0">
                        <label for="email" class="text-[#395670] font-bold">Alamat Email</label>
                        <input type="text" name="email" id="email" class="rounded" value="{{ old('email', $data->email) }}" required>
                    </div>
                </div>
                <div class="md:flex md:gap-10">
                    <div class="basis-1/2 flex flex-col mt-2 md:mt-0">
                        <label for="password" class="text-[#395670] font-bold">Kata Sadi Baru</label>
                        <input type="text" name="password" id="password" class="rounded" value="{{ old('password') }}">
                    </div>
                    <div class="basis-1/2 flex flex-col mt-2 md:mt-0">
                        <label for="password_confirmation" class="text-[#395670] font-bold">Konfirmasi Kata Sandi Baru</label>
                        <input type="text" name="password_confirmation" id="password_confirmation" class="rounded" value="{{ old('passwrord_confirmation') }}">
                    </div>
                </div>
            </div>
        </div>
