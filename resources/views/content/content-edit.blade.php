...
@section('content')
    <div class="border-s-4 border-[#395670] text-3xl font-bold text-[#395670] px-3">Ubah Materi</div>
    ...
    <form action="{{ route('materi.update', $data->id) }}" method="post" enctype="multipart/form-data" class="w-full pb-7">
        @csrf
        @method('PUT')
        <div class="md:flex">
            <div class="basis-3/4 md:flex md:flex-col md:gap-5 md:pe-5">
                <div class="md:flex md:gap-10">
                    <div class="basis-1/2 flex flex-col">
                        <label for="title" class="text-[#395670] font-bold">Nama Materi</label>
                        <input type="text" name="title" id="title" class="rounded" required value="{{ old('title', $data->title) }}">
                    </div>
                    <div class="basis-1/2 flex flex-col mt-3 md:mt-0">
                        <label for="video_link" class="text-[#395670] font-bold">Link Video</label>
                        <input type="text" name="video_link" id="video_link" class="rounded" required value="{{ old('video_link', $data->video_link) }}">
                    </div>
                </div>
                <div class="flex flex-col mt-3 md:mt-0">
                    <label for="preview" class="text-[#395670] font-bold">Preview (Ringkasan)</label>
                    <input type="text" name="preview" id="preview" class="rounded" value="{{ old('preview', $data->preview) }}" maxlength="200">
                </div>
                <div class="flex flex-col mt-3 md:mt-0">
                    <label for="body" class="text-[#395670] font-bold">Isi Materi</label>
                    <input type="hidden" name="body" id="body">
                    <trix-editor input="body" required class="min-h-[25vh]">{!! old('body', $data->body) !!}</trix-editor>
                </div>
            </div>
    
            <div class="md:basis-1/4 md:px-2 mt-3 md:mt-0">
                <label for="thumbnail_path" class="text-[#395670] font-bold">Banner Materi (Tarik atau klik untuk upload file)</label>
                <input type="file" id="dropify" name="thumbnail_path" class="dropify border-2" accept="image/*" data-default-file="{{ asset('/storage/materi/thumbnail/' . $data->thumbnail_path) }}"> 
            </div>
        </div>
